...
export default function EventsCalendar({ goals = [], className = '', onNavigate }: EventsCalendarProps) {
  ...
  return (
    <div
      ...
      className={`overflow-hidden rounded-[14px] border border-slate-200 bg-white ${className}`}
    >
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white p-5">
        <div>
          <h2 className="flex items-center gap-2.5 text-xl font-semibold text-slate-950">
            <CalendarIcon className="h-5 w-5 text-indigo-600" />
            Calendario strategico
          </h2>
          <p className="text-xs text-gray-500 mt-0.5">
            {events.filter(e => !e.completed).length} event{events.filter(e => !e.completed).length === 1 ? 'o attivo' : 'i attivi'}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="inline-flex min-h-[40px] items-center gap-2 rounded-[10px] bg-indigo-600 px-4 text-sm font-semibold text-white transition-colors hover:bg-indigo-700"
          >
      ...
      <div className="flex flex-col xl:flex-row">
        ...
        <div className="flex-1 p-5">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="min-h-[36px] rounded-lg border border-slate-200 px-3 text-xs font-semibold text-indigo-700 transition-colors hover:bg-indigo-50"
              >
                Oggi
              </button>
            </div>
            ...
            {calendarDays.map(({ date, isCurrentMonth }, i) => {
              ...
              return (
                <button key={i} onClick={() => setSelectedDate(key === selectedDate ? null : key)}
                  ...
                  className={`relative min-h-[76px] rounded-lg border p-1.5 text-left transition-colors
                  ...
                  {dayEvents.length > 0 && (
                    <div className="mt-1 space-y-0.5">
                      {dayEvents.slice(0, 2).map((e, j) => {
                        ...
                        return (
                          <div key={j} className={`truncate rounded px-1 py-0.5 text-[11px] font-medium ${
          ...
          {events.length === 0 && (
            <div
              ...
              className="mt-5 rounded-xl border border-dashed border-slate-300 bg-slate-50/70 px-6 py-6 text-center"
            >
              <CalendarIcon className="w-10 h-10 text-blue-500/70 mx-auto mb-3" />
              <h3 className="text-base font-bold text-gray-900">
                Nessun evento strategico pianificato
              </h3>
              <p className="mt-1 text-sm text-gray-600 max-w-md mx-auto">
                Aggiungi scadenze, esami, incontri o milestone. I TimeBlock di esecuzione restano nel Time Planner.
              </p>
        ...
        {/* Right Panel */}
        <aside className="border-t border-slate-200 bg-slate-50/70 xl:w-80 xl:border-l xl:border-t-0" aria-label="Agenda contestuale">
          <div className="p-5 space-y-6">
            {/* Next Key Event */}
            {nextKeyEvent && (
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-widest text-indigo-700">Prossimo evento</div>
                ...
                {nextKeyEvent.priority === 'high' && (
                  <div className="flex items-center gap-1 mt-2 text-xs text-rose-700 font-medium">
                    <AlertTriangle className="w-3 h-3" /> Priorità alta
                  </div>
                )}
              </div>
            )}
              ...
              <div>
                ...
                {selectedDayEvents.length === 0 ? (
                  <div className="text-center py-4">
                    <p className="text-xs text-gray-500">Nessun evento in questo giorno</p>
                    <button onClick={() => { setEditingEvent(null); setShowCreateModal(true); }}
                      className="mt-2 text-xs font-medium text-indigo-700 hover:text-indigo-900">+ Aggiungi evento</button>
                  </div>
                ) : (
            ...
            {thisWeekEvents.length > 0 && (
              <div>
                <h3 className="mb-3 text-xs font-bold uppercase tracking-widest text-gray-500">Questa settimana</h3>
                <div className="space-y-2">
            ...
            {upcomingEvents.length === 0 && !selectedDate && events.length > 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-sm font-medium text-gray-700">Nessun evento futuro</p>
                <p className="mt-1 text-xs text-gray-500">Gli eventi passati restano disponibili.</p>
              </div>
            )}
          </div>
        </aside>
