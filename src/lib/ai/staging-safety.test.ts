...
const SAFE_ENV = {
  ...
  LIFE_TRACKER_STAGING_FIREBASE_PROJECT_ID: 'life-tracker-staging-1',
  LIFE_TRACKER_STAGING_FIREBASE_API_KEY: 'public-web-api-key',
  LIFE_TRACKER_STAGING_FIREBASE_AUTH_DOMAIN: 'life-tracker-staging-1.firebaseapp.com',
  LIFE_TRACKER_STAGING_FIREBASE_APP_ID: '1:123:web:abc',
  LIFE_TRACKER_STAGING_FIREBASE_MESSAGING_SENDER_ID: '123',
  LIFE_TRACKER_STAGING_AI_API_BASE_URL:
    'https://europe-west1-life-tracker-staging-1.cloudfunctions.net/lifeTrackerAiApi',
} as const;
...
describe('live staging safety gate', () => {
  it('accepts one explicit, project-bound non-production configuration', () => {
    expect(readStagingEnvironment(SAFE_ENV)).toMatchObject({
      projectId: 'life-tracker-staging-1',
  ...
  it.each(['life-tracker-12000', 'life-tracker-test'])(
    'refuses the forbidden project %s',
    (projectId) => {
      ...
      })).toThrow(/not an eligible dedicated staging project/);
    },
  );
