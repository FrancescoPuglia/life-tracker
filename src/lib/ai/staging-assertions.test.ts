...
describe('secret-safe staging evidence assertions', () => {
  ...
  it('accepts exactly one canonical WPI marker line inside server-generated notes', () => {
    const marked = plan();
    marked.diff[0]!.changedFields = ['startTime', 'endTime', 'notes'];
    Object.assign(marked.diff[0]!.after, {
      notes: 'Generated by Weekly Planning Intelligence.\nSource: preview_timeblock_change:block-1\nWPI_KEY: wpi:ai_draft_0123456789abcdefabcd:block-1',
    });
    expect(() => requireExactPlan(200, marked, {
      tool: 'preview_timeblock_change',
      action: 'move',
      entityType: 'timeBlocks',
      entityId: 'block-1',
      ...
      startTime: START,
      endTime: END,
      changedFields: ['startTime', 'endTime', 'notes'],
      beforeFields: { id: 'block-1', title: 'Block' },
      afterFields: { id: 'block-1', title: 'Block', startTime: START, endTime: END },
  ...
  it.each([
    'WPI_KEY: wpi:ai_draft_0123456789abcdefabcd:block-1\nWPI_KEY: wpi:ai_draft_0123456789abcdefabcd:block-1',
    'ordinary text WPI_KEY: wpi:ai_draft_0123456789abcdefabcd:block-1',
    'WPI_KEY: wpi:ai_draft_0123456789abcdefabcd:other-block',
  ])('rejects duplicate, inline, or wrong-entity WPI marker evidence', (notes) => {
    const marked = plan();
    marked.diff[0]!.changedFields = ['startTime', 'endTime', 'notes'];
    Object.assign(marked.diff[0]!.after, { notes });
    expect(() => requireExactPlan(200, marked, {
      tool: 'preview_timeblock_change',
      action: 'move',
      entityType: 'timeBlocks',
      ...
      afterFields: { id: 'block-1', title: 'Block', startTime: START, endTime: END },
      requiresWpiMarker: true,
    })).toThrow('Staging proposal did not contain exactly one server-generated WPI marker.');
  });
