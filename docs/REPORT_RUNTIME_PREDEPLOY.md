...
Resend credential was read and no email was sent.
...
| `deliverScheduledScientificReports` | Cloud Scheduler every five minutes in UTC | 540 s, 1 GiB, min 0, max 1, concurrency 1, 3 platform retries within 15 minutes | `RESEND_API_KEY` only |
...
Server state is limited to two stable paths per owner:
...
4. Secret Manager **metadata only** for `RESEND_API_KEY` (existence, versions,
   replication, IAM), never its value;
...
3. Store non-secret runtime values only in the ignored target-specific Functions
   ...
   UID or mailbox.
4. Keep `REPORT_EMAIL_RUNTIME_ENABLED=false` while deploying the named
   Functions, reviewed Rules, and index.
...
The first-line stop is setting the runtime switch back to exact `false`; both
handlers then stop before owner/Firestore/provider access. If a code rollback is
needed, redeploy the recorded prior Function revision/source and restore the
recorded Rules/index metadata. Do not delete report archives, email delivery
attempts, run documents, or real user data as part of rollback. Queued Scheduler
